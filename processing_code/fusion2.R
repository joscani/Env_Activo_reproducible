...
################################
# Incidencias para revisar:
# - Módulo AC: No están variables ac004d3_ (1:7) ni ac004d6_(1:7)
# - Módulo DN: Falta variable gender
...
sel3 <- expand.grid(paste0("ac004d",c(1,2,4,5,7)),paste0("_",1:7))
...
# pongo gender en dn042_

names(unido2) [names(unido2)  %in% "dn042_"] <- "gender"
...
cogn1 <- ifelse(as.numeric(ola2$cf003_)==3,1,0)
cogn1[is.na(ola2$cf003_)] <- 0

cogn2 <- ifelse(as.numeric(ola2$cf004_)==3,1,0)
cogn2[is.na(ola2$cf004_)] <- 0

cogn3 <- ifelse(as.numeric(ola2$cf005_)==3,1,0)
cogn3[is.na(ola2$cf005_)] <- 0

cogn4 <- ifelse(as.numeric(ola2$cf006_)==3,1,0)
cogn4[is.na(ola2$cf006_)] <- 0

comp1 <- cogn1+cogn2+cogn3+cogn4
comp2 <- with(ola2,cf008tot+cf016tot)
comp2[is.na(comp2)] <- 0
...
ola2$cognitive <- cognitive

if(!file.exists("tempData")){
	dir.create("tempData")
}

if(!file.exists("tempData/ola2.RData")){
	save(ola2,file="tempData/ola2.RData")
}
rm(list=ls())
